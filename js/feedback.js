...
function contactSend() {
	...
	document.querySelector('.contact__button').addEventListener('click', event => {
		...
		if ( document.querySelectorAll('.contact__input')[0].value.length < 2 ) {
		...
		} else if ( document.querySelectorAll('.contact__input')[2].value.length < 10 || !document.querySelectorAll('.contact__input')[2].value.includes("@") || !document.querySelectorAll('.contact__input')[2].value.includes(".") ) {
			document.querySelector('.contact__message').textContent = 'Enter your e-mail';
			document.querySelector('.contact__message').style.maxHeight = '60px';
			document.querySelector('.contact__message').style.marginBottom = '20px';
			document.querySelector('.contact__message').style.color = '#ff4a4a';
		...
		} else if ( document.querySelectorAll('.contact__input')[3].value.length < 10 ) {
			document.querySelector('.contact__message').textContent = 'Write your message (minimum 10 characters)';
			document.querySelector('.contact__message').style.maxHeight = '60px';
			document.querySelector('.contact__message').style.marginBottom = '20px';
			document.querySelector('.contact__message').style.color = '#ff4a4a';
			document.querySelectorAll('.contact__input')[3].style.background = "#ffc1c1";
			setTimeout( () => {
				document.querySelector('.contact__message').style.maxHeight = '0px';
				document.querySelector('.contact__message').style.marginBottom = '0px';
				document.querySelectorAll('.contact__input')[3].style.background = "#fff";
			}, 2000)
		} else {
			...
			messageTg += `<b>✉️ E-mail - </b> ${document.querySelectorAll('.contact__input')[2].value}\n`;
			messageTg += `<b>✏️ Сообщение - </b> ${document.querySelectorAll('.contact__input')[3].value}\n`;
			...
			document.querySelectorAll('.contact__input')[3].value = "";
